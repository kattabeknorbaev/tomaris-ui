      ...
      <div className="border-b border-border bg-canvas/80 backdrop-blur-xl">
        <div className="mx-auto flex h-14 max-w-5xl items-center justify-between px-5 sm:px-8">
          ...
          <nav className="hidden items-center gap-0.5 md:flex">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} className="rounded-md px-3 py-1.5 text-body-sm text-body transition-colors duration-150 hover:text-ink">
                {link.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="hidden items-center gap-2 md:flex">
            ...
            <Link href="/login" className="rounded-md px-3 py-1.5 text-body-sm text-body transition-colors duration-150 hover:text-ink">
              {t.nav.login}
            </Link>
          ...
          <button className="flex h-8 w-8 items-center justify-center rounded-md text-body md:hidden" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Toggle menu">
        ...
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.15 }} className="border-b border-border bg-canvas/95 backdrop-blur-xl md:hidden overflow-hidden">
            <div className="px-5 py-4 space-y-1">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)} className="block rounded-md px-3 py-2.5 text-body-sm text-body hover:text-ink transition-colors duration-150">
                  {link.label}
                </Link>
              ))}
