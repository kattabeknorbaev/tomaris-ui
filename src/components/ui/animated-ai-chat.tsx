    ...
    return (
        ...
        <div className="absolute inset-0 w-full h-full overflow-hidden">
                    ...
                    >
                        ...
                        <div className="p-4">
                            <Textarea
                                ...
                                placeholder="Ask Tomaris a question..."
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-2">
                        {commandSuggestions.map((suggestion, index) => (
                            <motion.button
                                key={suggestion.prefix}
                                onClick={() => selectCommandSuggestion(index)}
                                className="flex items-center gap-2 px-3 py-2 bg-card hover:bg-muted rounded-lg text-sm text-muted-foreground hover:text-foreground transition-all relative group"
                                initial={{ opacity: 0, y: 10 }}
                                animate={{ opacity: 1, y: 0 }}
            ...
            <AnimatePresence>
                {isTyping && (
                    <motion.div 
                        className="fixed bottom-8 mx-auto transform -translate-x-1/2 backdrop-blur-2xl bg-card/80 rounded-full px-4 py-2 shadow-lg border border-border"
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 20 }}
                    >
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-7 rounded-full bg-primary/10 flex items-center justify-center text-center">
                                <span className="text-xs font-medium text-primary mb-0.5">T</span>
                            </div>
            ...
            {inputFocused && (
                <motion.div 
                    className="fixed w-[50rem] h-[50rem] rounded-full pointer-events-none z-0 opacity-[0.03] bg-gradient-to-r from-primary via-accent to-primary blur-[96px]"
                    animate={{
        ...
        <div className="flex items-center ml-1">
            {[1, 2, 3].map((dot) => (
                <motion.div
                    ...
                    style={{
                        boxShadow: "0 0 4px rgba(15, 143, 111, 0.3)"
                    }}
                />
            ))}
        </div>
...
function ActionButton({ icon, label }: ActionButtonProps) {
    ...
    return (
        <motion.button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-card hover:bg-muted rounded-full border border-border text-muted-foreground hover:text-foreground transition-all relative overflow-hidden group"
        >
